#!/usr/bin/env bash
# set -e
export AVAILABILITY_ZONE=$(curl -sLf http://169.xxxx/latest/meta-data/placement/availability-zone)
export INSTANCE_ID=$(curl -sLf http://169.2xxx/latest/meta-data/instance-id)
export AWS_REGION=$(curl -s 169.254.1xxx4/latest/meta-data/placement/availability-zone | sed 's/.$//')
...
hostname ${NEW_HOSTNAME}-${INSTANCE_ID}
cat >> /etc/cloud/cloud.cfg << EOF
# Preserve hostname across reboots
preserve_hostname: true
EOF
hostnamectl set-hostname ${NEW_HOSTNAME}-${INSTANCE_ID}
...
#***********************************
# Mount or build Splunk app volume *
#***********************************
...
echo "#### Mounting/building Splunk volume..."
...
EOF
         ...
         # Set ulimits for Splunkd service (override what was created in splunk-base-ami)
         mkdir -p /etc/systemd/system/Splunkd.service.d
         cat > /etc/systemd/system/Splunkd.service.d/override.conf << EOF
[Service]
# 64K per pipeline
LimitNOFILE=262144
LimitNPROC=16000
...
##################################
# Mount or build the Data volume #
##################################

# Find and attach, or create a new volume for /data
# First, look for an existing volume with a Name of ${NEW_HOSTNAME}:app (e.g. "splunk-utility:data").
# If we find one, forcibly detach it from any instance it is currently attached to, then attach it to me.
# If we don't find one, then create a new volume and install Splunk on it. (This should only be the first time.)
echo "#### Mounting/building Data volume..."
current_volume=$(aws ec2 describe-volumes --region ${AWS_REGION} --filters Name=tag:Name,Values=${NEW_HOSTNAME}:data --query "Volumes[*].VolumeId" --output text)
...
if [ -n "$current_volume" ]; then
    ...
    retries=5
    for ((i=0; i<retries; i++)); do
        aws ec2 attach-volume --region ${AWS_REGION} --device /dev/xvdc --instance-id ${INSTANCE_ID} --volume-id ${current_volume}
        [[ $? -eq 0 ]] && break
    ...
    if [ retries -eq i ]; then
        ...
        echo "Mounted volume ${current_volume} as /dev/nvme2n1 on /data."
    fi
fi

if [ ! -b "/dev/nvme2n1" ]; then
    # Finally, create a new volume.
    echo "No existing volume found. Creating new one."
    new_volume_json=$(aws ec2 create-volume --region ${AWS_REGION} --availability-zone ${AVAILABILITY_ZONE} --volume-type gp3 --iops 16000 --size 5120 --tag-specifications "ResourceType=volume,Tags=[{Key=Name,Value=\"${NEW_HOSTNAME}:data\"}]")
    echo ${new_volume_json}
    new_volume_id=$(echo $new_volume_json | grep -oP '"VolumeId": "\K[^"]*')
    echo New Volume ID: ${new_volume_id}
    aws ec2 modify-volume --region ${AWS_REGION} --volume-id ${new_volume_id} --throughput 1000
    # Wait to let the volume get created. Try 10 times with 10 seconds in-between each.
    retries=5
    for ((i=0; i<retries; i++)); do
        aws ec2 attach-volume --region ${AWS_REGION} --device /dev/xvdc --instance-id ${INSTANCE_ID} --volume-id ${new_volume_id}
        [[ $? -eq 0 ]] && break
    ...
    if [ -b "/dev/nvme2n1" ]; then
        ...
        # Mount it! (Don't need to set it up in /etc/fstab, because if this instance ever reboots, ASG will kick in and build a new instance!)
        mount /dev/nvme2n1 /data
        echo "Mounted volume /dev/nvme2n1 to /data."
    else
        echo "Could not attach volume!!!"
    fi
fi

# Set ownership of /data to splunk user (this is because UIDs could be different)
chown -R splunk:splunk /data

# Set up /etc/fstab so that volumes will auto-mount if indexer is restarted. Really only needed for Dev.
echo "UUID=$(lsblk /dev/nvme2n1 -n -o UUID)   /data    xfs    defaults,nofail    0    2" >> /etc/fstab
...
aws cloudwatch put-metric-alarm --alarm-name "${NEW_HOSTNAME}_disk_used_percent_Warning" --metric-name "disk_used_percent" --alarm-description "Warning: Disk Used is above 95% on ${NEW_HOSTNAME} in ${AWS_ALIAS}." --namespace CWAgent --statistic Average --period 60 --threshold 95 --comparison-operator GreaterThanThreshold --dimensions Name=InstanceId,Value=${INSTANCE_ID} Name=path,Value=/ Name=device,Value=nvme0n1p1 Name=fstype,Value=xfs --evaluation-periods 5 --alarm-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --ok-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --region ${AWS_REGION} --tags Key=Hostname,Value=${NEW_HOSTNAME} Key=AccountAlias,Value=${AWS_ALIAS}
aws cloudwatch put-metric-alarm --alarm-name "${NEW_HOSTNAME}_disk_used_percent_Error" --metric-name "disk_used_percent" --alarm-description "Error: Disk Used is above 96% on ${NEW_HOSTNAME} in ${AWS_ALIAS}." --namespace CWAgent --statistic Average --period 60 --threshold 96 --comparison-operator GreaterThanThreshold --dimensions Name=InstanceId,Value=${INSTANCE_ID} Name=path,Value=/ Name=device,Value=nvme0n1p1 Name=fstype,Value=xfs --evaluation-periods 5 --alarm-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --ok-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --region ${AWS_REGION} --tags Key=Hostname,Value=${NEW_HOSTNAME} Key=AccountAlias,Value=${AWS_ALIAS}
aws cloudwatch put-metric-alarm --alarm-name "${NEW_HOSTNAME}_disk_used_percent_Critical" --metric-name "disk_used_percent" --alarm-description "Critical: Disk Used is above 97% on ${NEW_HOSTNAME} in ${AWS_ALIAS}." --namespace CWAgent --statistic Average --period 60 --threshold 97 --comparison-operator GreaterThanThreshold --dimensions Name=InstanceId,Value=${INSTANCE_ID} Name=path,Value=/ Name=device,Value=nvme0n1p1 Name=fstype,Value=xfs --evaluation-periods 5 --alarm-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --ok-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --region ${AWS_REGION} --tags Key=Hostname,Value=${NEW_HOSTNAME} Key=AccountAlias,Value=${AWS_ALIAS}

aws cloudwatch put-metric-alarm --alarm-name "${NEW_HOSTNAME}_disk_free_Warning_data" --metric-name "disk_free" --alarm-description "Warning: Disk Free space for /data is below 10GB on ${NEW_HOSTNAME} in ${AWS_ALIAS}." --namespace CWAgent --statistic Average --period 60 --threshold 10737418240 --comparison-operator LessThanThreshold --dimensions Name=InstanceId,Value=${INSTANCE_ID} Name=path,Value=/data Name=device,Value=nvme2n1 Name=fstype,Value=xfs --evaluation-periods 5 --alarm-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --region ${AWS_REGION} --ok-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --region ${AWS_REGION} --tags Key=Hostname,Value=${NEW_HOSTNAME} Key=AccountAlias,Value=${AWS_ALIAS}
aws cloudwatch put-metric-alarm --alarm-name "${NEW_HOSTNAME}_disk_free_Critical_data" --metric-name "disk_free" --alarm-description "Critical: Disk Free space for /data is below 5GB on ${NEW_HOSTNAME} in ${AWS_ALIAS}." --namespace CWAgent --statistic Average --period 60 --threshold 5368709120 --comparison-operator LessThanThreshold --dimensions Name=InstanceId,Value=${INSTANCE_ID} Name=path,Value=/data Name=device,Value=nvme2n1 Name=fstype,Value=xfs --evaluation-periods 5 --alarm-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --region ${AWS_REGION} --ok-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --region ${AWS_REGION} --tags Key=Hostname,Value=${NEW_HOSTNAME} Key=AccountAlias,Value=${AWS_ALIAS}

aws cloudwatch put-metric-alarm --alarm-name "${NEW_HOSTNAME}_CPUUtilization_Warning" --metric-name "CPUUtilization" --alarm-description "Warning: CPU Utilization is above 90% on ${NEW_HOSTNAME} in ${AWS_ALIAS}." --namespace AWS/EC2 --statistic Average --period 60 --threshold 90 --comparison-operator GreaterThanThreshold --dimensions Name=InstanceId,Value=${INSTANCE_ID} --evaluation-periods 5 --alarm-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --ok-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --region ${AWS_REGION} --tags Key=Hostname,Value=${NEW_HOSTNAME} Key=AccountAlias,Value=${AWS_ALIAS}
aws cloudwatch put-metric-alarm --alarm-name "${NEW_HOSTNAME}_CPUUtilization_Error" --metric-name "CPUUtilization" --alarm-description "Error: CPU Utilization is above 98% on ${NEW_HOSTNAME} in ${AWS_ALIAS}." --namespace AWS/EC2 --statistic Average --period 60 --threshold 98 --comparison-operator GreaterThanThreshold --dimensions Name=InstanceId,Value=${INSTANCE_ID} --evaluation-periods 5 --alarm-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --ok-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --region ${AWS_REGION} --tags Key=Hostname,Value=${NEW_HOSTNAME} Key=AccountAlias,Value=${AWS_ALIAS}

aws cloudwatch put-metric-alarm --alarm-name "${NEW_HOSTNAME}_swap_used_percent_Warning" --metric-name "swap_used_percent" --alarm-description "Warning: Swap used is above 75% on ${NEW_HOSTNAME} in ${AWS_ALIAS}." --namespace CWAgent --statistic Average --period 60 --threshold 75 --comparison-operator GreaterThanThreshold --dimensions Name=InstanceId,Value=${INSTANCE_ID} --evaluation-periods 5 --alarm-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --ok-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --region ${AWS_REGION} --tags Key=Hostname,Value=${NEW_HOSTNAME} Key=AccountAlias,Value=${AWS_ALIAS}
aws cloudwatch put-metric-alarm --alarm-name "${NEW_HOSTNAME}_swap_used_percent_Error" --metric-name "swap_used_percent" --alarm-description "Error: Swap used is above 55% on ${NEW_HOSTNAME} in ${AWS_ALIAS}." --namespace CWAgent --statistic Average --period 60 --threshold 85 --comparison-operator GreaterThanThreshold --dimensions Name=InstanceId,Value=${INSTANCE_ID} --evaluation-periods 5 --alarm-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --ok-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --region ${AWS_REGION} --tags Key=Hostname,Value=${NEW_HOSTNAME} Key=AccountAlias,Value=${AWS_ALIAS}
aws cloudwatch put-metric-alarm --alarm-name "${NEW_HOSTNAME}_swap_used_percent_Critical" --metric-name "swap_used_percent" --alarm-description "Critical: Swap used is above 95% on ${NEW_HOSTNAME} in ${AWS_ALIAS}." --namespace CWAgent --statistic Average --period 60 --threshold 95 --comparison-operator GreaterThanThreshold --dimensions Name=InstanceId,Value=${INSTANCE_ID} --evaluation-periods 5 --alarm-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --ok-actions arn:aws:sns:${AWS_REGION}:${AWS_ACCOUNT}:splunk-alarms --region ${AWS_REGION} --tags Key=Hostname,Value=${NEW_HOSTNAME} Key=AccountAlias,Value=${AWS_ALIAS}
